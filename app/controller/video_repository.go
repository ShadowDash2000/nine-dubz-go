...
func (vr *VideoRepository) SplitVideoToThumbnails(filePath string, outputPath string) error {
	...
	for second := 1; second < duration; second = second + 10 {
		...
		ffmpeg.
			...
			Output(imagePath, ffmpeg.KwArgs{"vframes": "1"}).
			Silent(true).
			Run()
...
func (vr *VideoRepository) Resize(filePath string, outputPath string, fileName string) error {
	err := os.MkdirAll(outputPath, os.ModePerm)
	if err != nil {
		return err
	}
